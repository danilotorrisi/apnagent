 ...
 * ### .silent (bool)
 *
 * Enable the silent notification.
 *
 * ```js
 * msg.silent(true)
 * ```
 *
 * @param {Bool} true if want to enable silent notification.
 * @returns {this} for chaining
 * @api public
 * @name silent
 */
Message.prototype.silent = function (enable) {
  
  // If want to enable.
  if (enable) {

    // Add the content-available key to aps.
    this.aps['content-available'] = 1;

  } else if (this.aps['content-available'])  {

    // Otherwise if want to disable and already enabled.
